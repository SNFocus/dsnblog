...
export let articleRoutes = routes1.keys().map(key => {
...
export let pageRoutes = routes3.keys().map(key => {
    ...
    if (name === 'demo') {
        let demoChildren = routes2.keys().map(key => {
            let name = key.replace("./", '').replace('.vue', '').replace(/\/index$/, '')
            // .match(/(?<=\/)[\w|-]*(?=\.md)/)[0].toLowerCase()
            return {
                path: name,
        ...
        return {
            ...
            children: [...demoChildren, { path: '', component: demoChildren[0].component }]
        }
    }
